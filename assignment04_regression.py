...
def load_reit_annual_data(data_path: Path) -> pd.DataFrame:
    ...
    # Rename ret12 to ret (annual return = 12-month cumulative return)
...
def plot_scatter_with_regression(
    ...
    y_min = plot_df["ret"].quantile(0.02)
    y_max = plot_df["ret"].quantile(0.98)
    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_min, y_max)
    
    # Add title with R², xlabel, ylabel, legend
    r2 = model.rsquared
...
def print_key_results(model, x_var: str) -> None:
    ...
    # Print intercept and slope with standard errors, t-stats, p-values
    print(f"Intercept (β₀):  {intercept:10.6f}  (SE: {intercept_se:.6f}, t: {intercept_tstat:7.3f}, p: {intercept_pval:.6f})")
    print(f"Slope (β₁):      {slope:10.6f}  (SE: {slope_se:.6f}, t: {slope_tstat:7.3f}, p: {slope_pval:.6f})")
    
    # Print R², Adjusted R², N
    print(f"\nR²:              {r2:.6f}")
    ...
    is_significant = "Yes" if slope_pval < 0.05 else "No"
    print(f"\nSlope is {slope_direction} and significant at 5%: {is_significant}")
